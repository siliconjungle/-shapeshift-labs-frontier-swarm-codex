...
Cost telemetry is priced in this adapter, not in the runtime-neutral `frontier-swarm` package. Known Codex models use the package-local `FRONTIER_SWARM_CODEX_MODEL_PRICING` catalog for uncached input, cached input, and output tokens. Unknown or missing model pricing is represented with `estimated: false`, `reason: "unknown-model-pricing"` or `reason: "missing-model"`, `unknownPricing`, and summary/model-level `costEstimateStatus: "unknown-pricing"` instead of a synthetic `$0` estimate. Dashboard totals only add jobs whose usage was actually priced, and fully unknown summaries omit `estimatedCostUsd`.

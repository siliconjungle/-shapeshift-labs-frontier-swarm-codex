...
frontier-swarm continue \
  --collection agent-runs/codex-swarm/run-1/collected \
  --routing-policy config/model-routing-policy.json \
  --outDir agent-runs/codex-swarm/run-1/continuation

frontier-swarm plan \
  --manifest inkwell/agent-ownership.json \
  --backlog agent-runs/codex-swarm/run-1/continuation/backlog.next.json \
  --routing-policy agent-runs/codex-swarm/run-1/continuation/model-routing-policy.next.json \
  --routing-mode fill \
  --outDir agent-runs/codex-swarm/run-2-plan
...
This package does not expose or proxy a hosted OpenRouter Fusion endpoint. The panel/fuser graph is the swarm-native equivalent pattern: a Frontier plan fans out a panel of Codex jobs across lanes or compute profiles, and `collect`, `query`, `score`, and `continue` act as the fuser over evidence, patch candidates, model-routing feedback, and child backlogs. One-call hosted behavior still needs a service wrapper that accepts one prompt or request, provisions the panel, waits for or streams worker results, synthesizes one final answer, and owns provider auth, routing, rate limits, persistence, and billing or quota policy.
...
## Recursive Agent Backlogs

Backlog-driven runs can ask workers to split a large epic, task group, feature entry, or decision entry into child work for a later session. Plan those decomposition jobs from a durable backlog instead of letting workers mutate the parent queue directly:

```sh
frontier-swarm plan \
  --manifest agent-ownership.json \
  --backlog backlog.json \
  --recursive-backlog true \
  --max-backlog-depth 1 \
  --child-artifact-path backlog-children.json \
  --outDir agent-runs/codex-swarm/plan
```

The planner turns open, triaged, ready, or planned epics and task groups into decomposition jobs, and turns runnable backlog entries into normal tasks. Decomposition jobs receive the child artifact path in `allowedWrites`, so child agents produce append-only backlog JSON in their isolated workspaces. A child backlog is ordinary Frontier swarm backlog JSON: use stable ids, link entries back to parent epics/groups/entries, include refs and verification, and leave the parent backlog untouched.

After the worker run, collect first, then promote child backlog files and model feedback into next-wave inputs:
...
frontier-swarm continue \
  --collection agent-runs/codex-swarm/run-1/collected \
  --backlog backlog.json \
  --routing-policy model-routing-policy.json \
  --manifest agent-ownership.json \
  --tasks current-work-queue.json \
  --outDir agent-runs/codex-swarm/run-1/continuation \
  --routing-mode fill
```

`continue` also accepts `--run <run-dir>` and collects it before continuing. By default it discovers child backlog files named `proof-route-backlog.json`, `proof-parent-recheck-backlog.json`, `backlog-children.json`, `child-backlog.json`, or `children-backlog.json`; pass `--child-backlog-name <file>` for project-specific filenames. It also discovers `human-action-answers.jsonl`, `human-action-answers.json`, `human-answers.jsonl`, or `human-answers.json` beside the collection/run, discovers the lifetime dashboard log at `agent-runs/loom-ui-human-actions/human-action-answers.jsonl`, and `--human-answers <file>` can point at an explicit answer file. It writes `backlog.next.json`, `model-routing-policy.next.json`, `human-actions.next.json`, `tasks.next.json` when a task source is supplied, `next-plan.json` when manifest/tasks are supplied, `proof-parent-recheck-results.json` when parent-proof recheck tasks are present, a `proof-parent-apply-candidates/` collection when those rechecks are ready for strict apply admission, and `continuation.json` with child backlog counts, routing feedback counts, next job ids, lane counts, terminal-outcome projection counts, human-answer resolution counts, proof parent-recheck/candidate counts, routed-job counts, changed-compute counts, tournament/adaptive summaries, and output paths. Terminal outcomes from `queue-outcome-model.json` are projected onto the next backlog and explicit task input: checked/applied/no-change/rejected work is closed, reruns stay ready, coordinator-review work stays out of new implementation jobs, answered human questions return to ready work with compact answer metadata on the projected task, and unanswered human questions/conflicts remain blocked without rewriting historical worker evidence. Use `next-wave` as a readable alias for `continue`; leave `--write` off until the `.next` files have been reviewed.
...
Each run writes event streams under `streams/`, a `coordinator-dashboard.json` snapshot, `pids.json`, `dependency-health.json`, workspace proofs, patch files, merge bundles, per-job `evidence/evidence.json`, `patch-intent.json`, `log-summary.json`, and job results with merge-readiness classification. The runner also creates a durable `frontier-queue` runtime by default: `queue-state.json`, `queue-events.jsonl`, and `queue-summary.json`. Scheduler-ready jobs are enqueued with stable dedupe keys, leased through `frontier-queue`, and completed or dead-lettered back into the same state so stale manifests cannot resurrect terminal work; pass `--queue-state <file>` to share that state across run directories or `--queue-state false` to disable it. Each settled job also writes a durable model telemetry projection: `model-telemetry.jsonl` plus `model-telemetry-summary.json`, with model/tier, lane/work kind, status, token/cost estimate, verification counts, semantic sidecar signal, and human-action counts; pass `--model-telemetry <file|false>` or `--model-telemetry-summary <file|false>` to redirect or disable it. Live model routing is enabled by default: each scheduler refill turns settled telemetry into `model-routing-policy.live.json`, `routing-controller.json`, and `routing-controller-history.json`, then reroutes only unstarted jobs while protecting active and completed jobs. Pass `--live-routing false` to disable it, `--live-routing-mode fill|override|observe` to control policy behavior, or the `--live-routing-policy`, `--live-routing-controller`, and `--live-routing-history` path flags to redirect artifacts. Worker verification commands run through `@shapeshift-labs/frontier-test/node` and write per-job `evidence/gates/gate-executions.jsonl` plus `evidence/gates/gate-summary.json`, so dashboards can read the same gate contract for worker verification and coordinator apply evidence. When a worker is genuinely blocked on a product choice, ambiguous requirement, or unsafe assumption, the prompt asks it to write `evidence/human-question.json` with a short code, clear question, context, requested answer, and options; the runner normalizes that into both merge metadata and a durable human-question broker: `human-actions.jsonl` plus `human-actions-state.json`. Collection, continuation, and dashboard snapshots read the broker state before falling back to older merge metadata, and `--human-action-events <file|false>` / `--human-action-state <file|false>` control those paths. The normalized evidence file records changed paths, semantic regions, command pass/fail summaries, source citations, semantic sidecar stats, and ready-to-port patch hunk counts. `patch-intent.json` is the short review surface: what changed, why, risk, exact verification, semantic import quality, warnings, and whether the patch is safe to port manually. Raw Codex stdout/stderr capture is compacted by default; `log-summary.json` records original bytes, written bytes, and truncation counts, and `--max-event-bytes` / `--max-stderr-bytes` can tune the limits. `discoverCodexHandoffArtifacts` scans job directories for `last-message.md`, debug handoffs, replay logs, watchpoints, traces, diagnostics, logs, evidence JSON, and patches; `runCodexJob` adds those paths to result evidence and `metadata.codexHandoffArtifacts` so coordinator dashboards can link directly to replay/debug artifacts. `frontier-swarm stop --run <run-dir>` reads the pid manifest and terminates live worker processes without manually hunting process state.

Dashboard snapshots estimate model cost from the OpenAI standard pricing catalog at <https://openai.com/api/pricing/>. The estimate uses uncached input, cached input, and output tokens when the Codex logs report them; when output tokens are missing, the snapshot marks the estimate as input-only and preserves micro-dollar precision so small local runs do not collapse to a literal zero in downstream views.
...
HTML/CSS browser proof routes use the same collection surface. Proof workers write `playwright-runtime-proof.json`; collection indexes it in `playwright-runtime-proof-artifacts.json`, readmits validator candidates through the optional `frontier-lang` HTML/CSS safe-merge validators in `playwright-proof-readmission.json`, and joins admitted readmissions back to their source worker bundle in `playwright-proof-parent-admission.json`. Collection also writes `proof-parent-recheck-backlog.json` for admitted parent links, and `continue` discovers it as child backlog input for the next wave. When those entries are present, continuation writes `proof-parent-recheck-results.json` by re-reading the parent merge bundle and running `git apply --check` against the current checkout. Ready rechecks are copied into a derived `proof-parent-apply-candidates/` collection with its own ready queue outcome model, so `frontier-swarm apply --collection <continuation>/proof-parent-apply-candidates` still uses strict apply admission. Operators can also use `frontier-swarm query --continuation <continuation-dir> --proof-parent-apply-candidate` and `frontier-swarm apply --continuation <continuation-dir>` without copying the internal candidate collection path. `frontier-swarm query` and the dashboard snapshot read that derived collection as a distinct proof-parent apply queue. Parent admissions are explicit recheck evidence: they can classify the original bundle as ready for strict apply admission, needing gates, stale/conflicting, blocked, or missing artifacts, while leases, gates, and apply admission still run before landing.
...
Use `frontier-swarm sync --run <run-dir|swarm-results.json|run-events.jsonl> --peer <run-dir|swarm-results.json|run-events.jsonl>` to exchange run logs between local machines, cloud workers, or copied run archives. The command uses `@shapeshift-labs/frontier-run/node` JSONL store sync, writes `run-sync-evidence.json` and `run-sync-history.jsonl` by default, updates `run-dashboard.json`, and reports pulled/pushed event counts plus conflicts. `frontier-swarm run`, `collect`, `continue`, and closed-loop `run --closed-loop` also accept `--sync-peer <path>`, `--run-sync-direction pull|push|bidirectional`, `--run-sync-evidence <file|false>`, and `--run-sync-history <file|false>`, so collection and next-wave planning can read peer events before deriving queue, admission, and dashboard projections.
...
For closed-loop model routing, a run now has two feedback moments. During execution, live routing consumes settled telemetry at scheduler refill time and can reroute pending jobs in the same run. At collection time, `collect` remains the archive boundary that writes both `strategy-tournament.json` and `tournament-adaptive-feedback.json`, and carries through the run-owned `model-telemetry-summary.json` plus live-routing artifacts for dashboard and continuation context. Then `frontier-swarm continue --collection <collection-dir> --routing-policy <current-policy.json> --outDir <continuation-dir>` appends model-routing feedback from the collected bundles and writes `<continuation-dir>/model-routing-policy.next.json`. The feedback includes model/tier, bucket, evidence quality, tournament recommendations, billable-token counts, estimated input/output cost, and whether the estimate is input-only or model-pricing is unknown, so the next policy can bias simple work toward cheaper profiles and reserve expensive models for work that pays off. When manifest/tasks are supplied, continuation feeds the generated policy into `next-plan.json` immediately and records `summary.nextJobRouting` with routed job ids, changed compute ids, matched feedback, matched cost signals, and matched preferences. Feed that next policy into later `frontier-swarm plan` or `frontier-swarm run` calls with `--routing-policy <continuation-dir>/model-routing-policy.next.json`; pair it with `--routing-mode fill|override|observe` when you want an explicit routing mode.

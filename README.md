...
HTML/CSS browser proof routes use the same collection surface. Proof workers write `playwright-runtime-proof.json`; collection indexes it in `playwright-runtime-proof-artifacts.json`, readmits validator candidates through the optional `frontier-lang` HTML/CSS safe-merge validators in `playwright-proof-readmission.json`, and joins admitted readmissions back to their source worker bundle in `playwright-proof-parent-admission.json`. Collection also writes `proof-parent-recheck-backlog.json` for admitted parent links, and `continue` discovers it as child backlog input for the next wave. Parent admissions are explicit recheck evidence: they say the original bundle can be reconsidered with the admitted proof, while stale checks, gates, and apply admission still run before landing.

...
async function testCollectedRun(tmp) {
  ...
  assert.ok(await exists(path.join(collection.outDir, 'run-graph.json')));
  ...
  assert.strictEqual(collection.runGraph.kind, 'frontier.swarm-codex.run-graph');
  assert.strictEqual(collection.runGraph.summary.jobCount, 1);
  assert.strictEqual(collection.runGraph.summary.candidateCount >= 1, true);
  assert.strictEqual(collection.runGraph.summary.evidenceCount >= 1, true);
  assert.strictEqual(collection.runGraph.summary.decisionCount >= 1, true);
  assert.strictEqual(collection.runGraph.indexes.byJobId['runtime-runtime-action'].includes('candidate:runtime-runtime-action'), true);

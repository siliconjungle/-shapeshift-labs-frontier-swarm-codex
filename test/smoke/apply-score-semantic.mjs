...
import { createCodexCalibrationAdaptiveFeedback } from '../../dist/index.js';
import {
  fs,
  path,
...
export async function testWeakSemanticAdmissionScore({ applyRepo, tmp, readyDir, readySemanticImport }) {
  ...
  assert.strictEqual(byJob.get('semantic-edit-mixed-review-job').semanticEvidence.cleanEligible, true);
  assert.strictEqual(byJob.get('semantic-edit-mixed-review-job').semanticEvidence.semanticEditOperationCleanEligible, false);
  assert.strictEqual(byJob.get('semantic-edit-mixed-review-job').semanticEvidence.semanticEditAdmission.status, 'auto-merge-candidate');
  ...
  await fs.writeFile(path.join(collection, 'apply-ledger', 'apply-ledger.json'), JSON.stringify({
    entries: [
      { jobId: 'semantic-edit-needs-port-portable-job', status: 'applied' },
      { jobId: 'semantic-edit-mixed-review-job', status: 'applied' }
    ]
  }, null, 2) + '\n');
  ...
  assert.ok(!calibrated.calibration.landedNeedsPortJobIds.includes('semantic-edit-mixed-review-job'));
  assert.ok(!calibrated.calibration.truePositiveNeedsPortJobIds.includes('semantic-edit-mixed-review-job'));
  assert.ok(!calibrated.calibration.semanticAutoMergeCandidateJobIds.includes('semantic-edit-mixed-review-job'));
  assert.strictEqual(calibrated.calibration.needsPortPrecision, 0);
  await assertStalePatchScoreRecalibratesFromLandedLedger({ collection, tmp });
}
...
async function assertStalePatchScoreRecalibratesFromLandedLedger({ collection, tmp }) {
  const staleCollection = path.join(tmp, 'stale-score-calibration');
  const score = JSON.parse(await fs.readFile(path.join(collection, 'patch-scores', 'patch-score.json'), 'utf8'));
  await fs.mkdir(path.join(staleCollection, 'patch-scores'), { recursive: true });
  await fs.mkdir(path.join(staleCollection, 'apply-ledger'), { recursive: true });
  await fs.writeFile(path.join(staleCollection, 'patch-scores', 'patch-score.json'), JSON.stringify({
    ...score,
    collectionDir: staleCollection,
    outDir: path.join(staleCollection, 'patch-scores'),
    calibration: {
      source: 'none',
      landedJobIds: [],
  ...
  await fs.writeFile(path.join(staleCollection, 'apply-ledger', 'apply-ledger.json'), JSON.stringify({
    entries: [
      { jobId: 'semantic-edit-needs-port-portable-job', status: 'applied' },
      { jobId: 'semantic-edit-mixed-review-job', status: 'applied' }
    ]
  }, null, 2) + '\n');
  const feedback = await createCodexCalibrationAdaptiveFeedback({
    collectionDir: staleCollection,
    generatedAt: 123
  });
  assert.strictEqual(feedback.calibration.source, 'apply-ledger');
  assert.strictEqual(feedback.calibration.summary.landed, 2);
  assert.strictEqual(feedback.calibration.summary.truePositiveNeedsPort, 0);
  assert.strictEqual(feedback.calibration.needsPortPrecision, 0);
  assert.ok(!feedback.observations.some((entry) => entry.kind === 'landed-needs-port'));
  assert.ok(!feedback.observations.some((entry) => entry.kind === 'strategy-underperforming'));
  assert.ok(!feedback.calibration.semanticAutoMergeCandidateJobIds.includes('semantic-edit-mixed-review-job'));
}

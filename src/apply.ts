...
export async function applyCodexSwarmCollection(input: FrontierCodexApplyInput): Promise<FrontierCodexApplyResult> {
  ...
  const augmented: FrontierCodexApplyResult = { ...result, ...evidence };
  await fs.writeFile(path.join(result.outDir, 'apply-ledger.json'), JSON.stringify(augmented, null, 2) + '\n');
  return augmented;
}

async function writeCodexApplyEvidence(result: FrontierCodexApplyResult): Promise<Pick<FrontierCodexApplyResult, 'gateExecutionsPath' | 'gateSummaryPath' | 'runEventsPath' | 'runDashboardPath' | 'evidence'>> {

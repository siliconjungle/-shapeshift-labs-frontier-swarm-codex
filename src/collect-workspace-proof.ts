...
import { FRONTIER_SWARM_GIT_WORKSPACE_PROOF_KIND } from '@shapeshift-labs/frontier-swarm-git';
...
export async function readFrontierCodexWorkspaceProof(file: string): Promise<FrontierCodexWorkspaceProof | undefined> {
  ...
  if (value.kind !== FRONTIER_SWARM_GIT_WORKSPACE_PROOF_KIND || !isObject(value.manifest)) return undefined;

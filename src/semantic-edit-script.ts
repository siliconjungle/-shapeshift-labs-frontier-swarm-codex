...
const REVIEW_REQUIRED_STATUSES = ['needs-review', 'needsReview', 'review-required', 'reviewRequired', 'ambiguous', 'ambiguous-edit', 'ambiguousEdit'];
const EVIDENCE_ONLY_STATUSES = ['evidence-only', 'evidenceOnly', 'review-only', 'reviewOnly'];
...
function normalizeSemanticEditScript(record: Record<string, unknown>): FrontierCodexSemanticEditScriptSummary {
  ...
  const admissionCounts = { ...numberRecord(summary.admission), ...numberRecord(admission), ...(status ? { [status]: 1 } : {}) };
  ...
  const reviewStatusCount = Math.max(sumKeys(byStatus, REVIEW_REQUIRED_STATUSES), sumKeys(admissionCounts, REVIEW_REQUIRED_STATUSES));
  const explicitReviewRequired = admission.reviewRequired === true ? 1 : 0;
  const rawReviewRequired = Math.max(explicitReviewRequired, nonNegativeNumber(summary.reviewRequired), reviewStatusCount);
  const evidenceOnly = sumKeys(admissionCounts, EVIDENCE_ONLY_STATUSES);
  const reviewRequired = explicitReviewRequired === 0 && reviewStatusCount === 0 && evidenceOnly > 0 && rawReviewRequired <= evidenceOnly
    ? 0
    : rawReviewRequired;
  return {
    total,
    operations: operationCount,
    autoMergeCandidates: Math.max(
      admission.autoMergeCandidate === true ? 1 : 0,
      maxCounter(summary.autoMergeCandidates, byStatus, admissionCounts, ['auto-merge-candidate', 'autoMergeCandidate'])
    ),
    ...
    reviewRequired,
    autoApplyCandidates: admission.autoApplyCandidate === true ? 1 : maxCounter(summary.autoApplyCandidates, byStatus, admissionCounts, ['auto-apply-candidate', 'autoApplyCandidate']),
    byStatus,
    byKind,
...
function sumKeys(record: Record<string, number>, keys: readonly string[]): number {
  const aliases = new Set(keys.flatMap((key) => [normalizedStatusKey(key), compactStatusKey(key)]));
  let sum = 0;

...
4. Collapse stale work before reading it deeply. If the patch is stale against head, touches superseded files, depends on an already rejected approach, or cannot apply cleanly because accepted work moved the surface, mark it `stale-against-head`. Queue a fresh shard only when the original objective is still valuable.
...
6. Escalate true human blockers last. A blocked result should name the human decision, owner, file or package surface, and exact question that the coordinator cannot answer locally.
...
- `blocked`: a true human blocker remains after the coordinator has ruled out auto-drain, manual port, rejection, stale closure, and rerun.

...
| `auto-drain/rerun-manifest.json` | Whole auto-drain pass | Follow-up swarm coordinators | Task-shaped canonical `items[]` for unresolved conflict-blocked, decision-rerun, queue-rerun, or stale-against-head work, mirrored to the supported `tasks[]` compatibility alias. Each item carries original task/job ids, source patch paths, bundle paths, target refs, reasons, `currentHead`, `sourceHead` when known, and source task scheduling metadata such as lane, layer, compute, priority, concurrency key, acceptance, verification, and owned/changed regions when the source bundle recorded it. Item objectives name known source heads alongside the current head. Evidence-only work is omitted. |
...
Explicit `frontier-swarm-codex autonomous-apply` and `frontier-swarm-codex drain` runs use the same rerun-manifest format when continuation work remains. The explicit drain result embeds `rerunManifest`, writes `<drain-out-dir>/rerun-manifest.json`, and reports `summary.rerunManifestCount` plus `summary.rerunTaskCount` when stale collection entries, queue-rerun assignments, `rerun` decisions, or `conflict-blocked` decisions need a fresh worker wave. If there is no continuation work, explicit drain leaves the manifest absent and reports zero rerun tasks. In explicit drain output, `sourceAutoDrainPath` points at the source `autonomous-apply.json` ledger for the drain pass.
...
| `rerunManifest.paths` | `auto-drain/rerun-manifest.json` | Seed the next focused rerun or conflict-resolution swarm directly from task-shaped `items[]`; `tasks[]` is a compatibility alias. Use `metadata.rerun.currentHead`, `metadata.rerun.sourceHead`, and `metadata.rerun.sourceHeads` to detect stale follow-up items. |

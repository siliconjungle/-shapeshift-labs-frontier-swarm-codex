...
| `auto-drain/rerun-manifest.json` | Whole auto-drain pass | Follow-up swarm coordinators | Task-shaped canonical `items[]` for unresolved conflict-blocked, decision-rerun, queue-rerun, or stale-against-head work, mirrored to the supported `tasks[]` compatibility alias. Empty `items[]`/`tasks[]` with `summary.terminalState === "drained"` is an explicit terminal success signal: the manifest exists and there is no rerun work. Each non-empty item carries original task/job ids, source patch paths, bundle paths, target refs, reasons, `currentHead`, `sourceHead` when known, and source task scheduling metadata such as lane, layer, compute, priority, concurrency key, acceptance, verification, and owned/changed regions when the source bundle recorded it. Item objectives name known source heads alongside the current head. Evidence-only work is omitted. |
...
Rerun manifests are lifecycle artifacts, not append-only review queues. The manifest builder collapses queue item, task, and job aliases through the latest autonomous decisions; an older stale, rerun, or conflict-blocked bundle is omitted once a newer `applied` or `committed` decision satisfies the same queue subject. Open items preserve both original worker patch/bundle links and copied collection or apply artifact links so the next focused swarm can rerun from current head without rediscovering the source bundle. `items[]` is the canonical task list; `tasks[]` is the same list for consumers that only understand task-set shaped input. `summary.terminalState` is `"rerun-required"` when `summary.taskCount > 0` and `"drained"` when the manifest exists with zero tasks. Dashboard summaries mirror this as `autoDrainArtifacts.rerunManifest.terminalState` and `autoDrainArtifacts.summary.rerunManifestTerminalState`; a metadata value of `"missing"` means no manifest was written or indexed, not that the run was drained. When source task metadata is available, the rerun item is itself a schedulable task: feeding it back through `plan --rerun-manifest` should keep the original lane, layer, compute, priority, concurrency key, acceptance, verification, and owned/changed region scope instead of falling back to an unassigned lane or default priority. Consumers should read top-level task fields such as `lane`, `layer`, `compute`, `concurrencyKey`, `priority`, and `verification`, with the same values mirrored under `metadata.rerun` for audit/debug views.
...
Explicit `frontier-swarm-codex autonomous-apply` and `frontier-swarm-codex drain` runs always use the same rerun-manifest format. The explicit drain result embeds `rerunManifest`, writes `<drain-out-dir>/rerun-manifest.json`, and reports `summary.rerunManifestCount`, `summary.rerunTaskCount`, and `summary.rerunManifestTerminalState`. When stale collection entries, queue-rerun assignments, `rerun` decisions, or `conflict-blocked` decisions need a fresh worker wave, `summary.rerunManifestTerminalState` is `"rerun-required"` and the manifest contains task items. If there is no continuation work, explicit drain still writes an empty manifest with `items: []`, `tasks: []`, `summary.taskCount: 0`, and `summary.terminalState: "drained"` so dashboards and planners can distinguish a completed drain from a missing manifest. In explicit drain output, `sourceAutoDrainPath` points at the source `autonomous-apply.json` ledger for the drain pass.
...
| `rerunManifest.paths` | `auto-drain/rerun-manifest.json` | Seed the next focused rerun or conflict-resolution swarm directly from task-shaped `items[]`; `tasks[]` is a compatibility alias. Use `rerunManifest.terminalState` before planning: `"drained"` means a present empty manifest closed the run, `"rerun-required"` means schedule the listed tasks, and `"missing"` means the path index did not see a manifest. Use `metadata.rerun.currentHead`, `metadata.rerun.sourceHead`, and `metadata.rerun.sourceHeads` to detect stale follow-up items. |
...
| Human-blocked decision | `auto-drain/auto-drain.json.summary.humanBlockedCount`, `auto-drain/auto-drain.json.summary.answeredHumanBlockedCount`, `coordinator-dashboard.json.humanQuestions.count`, `coordinator-dashboard.json.humanQuestions.jobIds`, `coordinator-dashboard.json.humanQuestions.taskIds`, and `coordinator-dashboard.json.humanQuestions.reasons` | `auto-drain/apply-NN/autonomous-apply.json` `decisions[].status === "human-blocked"`, matching decision-log records, and `auto-drain/human-answer-routing.json` when an answer has landed | Show as a true human question only when the decision reason names the missing authority, parent assignment, ownership decision, or policy/risk decision. A routed answer removes the decision from open `humanQuestions`; keep the original decision ledger append-only. |
| Queue true blocker | `autoDrainArtifacts.mergeQueue.blockCount`, `coordinator-dashboard.json.queueMetadata.actionCounts.trueBlockerCount`, `coordinator-dashboard.json.queueMetadata.queueHealth.trueBlockerCount`, and `auto-drain/auto-drain.json.summary.humanAnswerContinuationCount` | `auto-drain/collection-NN/hierarchical-merge-queue.json` assignments with `action === "block"` and reasons including `true-blocker`; answered queue blockers also appear in `auto-drain/human-answer-routing.json.routedContinuations[]` | Show separately from conflict-blocked. This is a pre-apply queue/planner blocker based on an already blocked bundle state. A matching answer routes it as a continuation and removes it from open true-blocker counts. |
...
Use root `queueHealth` or `queueMetadata.queueHealth` when a UI needs drill-down counters, trend charts, or diagnostic filters. Use root `mergeQueueHealth` or `queueMetadata.mergeQueueHealth` when a UI needs machine-readable subjects behind those counters: active leases, queue scopes, coordinator assignments, terminal decisions, open human questions, real conflicts, rerun candidates, and applied decisions. The projection is intentionally compact and points back to queue ids, queue keys, patch paths, bundle paths, and decision ids instead of embedding full merge bundles. Use `queueHealth.currentHeadConflictCount`, `queueHealth.deferredCoordinatorCount`, and `queueHealth.deferredPromoteCount` for coordinator-drain debt views. Use root `humanQuestions` or `queueMetadata.humanQuestions` only for explicit missing-authority questions that need a human answer. Do not convert `queueMetadata.bucketCounts.needsHumanPortCount`, `queueHealth.staleOrRerunCount`, `queueHealth.conflictBlockedDecisionCount`, `queueHealth.currentHeadConflictCount`, `queueHealth.deferredCoordinatorCount`, or `autoDrainArtifacts.grouping.staleAgainstHeadCount` into blocker badges. Blocker UI should come from `operatorSummary.status`, the `true-blockers` card/count, and explicit `humanQuestions`.
...
Collected bundles can include `metadata.frontierSwarmCodex.collection.head`, the Git head used for collection-time stale checking. Autonomous apply re-reads the current head after taking the repo-local lock and compares it with that collected head before mutation. If the head moved because an earlier bundle in the same locked pass was committed, the next candidate is revalidated instead of automatically rerun: `git apply --check` must still pass, the prior commit chain must explain the current head, and the candidate's semantic/path/repo lock keys must not conflict with the committed bundle keys. Disjoint candidates may then run gates and commit in the same pass. A patch that no longer checks records `conflict-blocked`; an unexplained head move or conflicting lease surface records `rerun`. Both outcomes map to `stale-against-head` queue overlay entries and should feed rerun or conflict-resolution work, not human blocker badges.
...
| Rerun swarm seed | `autoDrainArtifacts.rerunManifest.paths[0]` or explicit drain `rerunManifest.path` | First check `summary.terminalState` on the manifest or `rerunManifest.terminalState` in the path index. `"drained"` means no follow-up swarm is needed; `"rerun-required"` means pass `auto-drain/rerun-manifest.json` or `<drain-out-dir>/rerun-manifest.json` to the next task loader. Its top-level canonical `items[]` are task-shaped, with matching `tasks[]` for compatibility, and include source patch refs, bundle refs, target refs, reasons, original task ids, original job ids, queue ids, `currentHead`, per-item `sourceHead`/`sourceHeads`, and preserved scheduling fields when the source task or bundle provided them. |
